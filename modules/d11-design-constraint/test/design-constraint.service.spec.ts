// ═══════════════════════════════════════════════════════════════════════
// D11: Design Constraint Engine — Unit Tests | Phase 6 Tier X
// ═══════════════════════════════════════════════════════════════════════
...
describe('DesignConstraintEngineService', () => {
  let service: DesignConstraintEngineService;
  let constraintRuleRepo: ReturnType<typeof mockRepo>;
  let densityMapRepo: ReturnType<typeof mockRepo>;
  let emphasisMapRepo: ReturnType<typeof mockRepo>;
  let eventBus: typeof mockEventBus;

  beforeEach(async () => {
    constraintRuleRepo = mockRepo();
    densityMapRepo = mockRepo();
    emphasisMapRepo = mockRepo();
    eventBus = { publish: jest.fn(() => Promise.resolve()) };
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DesignConstraintEngineService,
        { provide: getRepositoryToken(ConstraintRule), useValue: constraintRuleRepo },
        { provide: getRepositoryToken(DensityMap), useValue: densityMapRepo },
        { provide: getRepositoryToken(EmphasisMap), useValue: emphasisMapRepo },
        { provide: 'EventBus', useValue: eventBus },
      ],
    }).compile();
    service = module.get<DesignConstraintEngineService>(DesignConstraintEngineService);
    (service as any).eventBus = eventBus;
  });

  describe('analyzeDesign', () => {
    it('should create with tenant isolation (TXD-001, P-016)', async () => {
      const result = await service.analyzeDesign('tenant-1', { name: 'Test' });
      expect(result).toBeDefined();
      expect(constraintRuleRepo.create).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'tenant-1' }));
    });
    it('should emit designconst.* event (ESR-003)', async () => {
      await service.analyzeDesign('tenant-1', { name: 'Test' });
      expect(eventBus.publish).toHaveBeenCalledWith(expect.stringContaining('designconst.'), expect.any(Object));
    });
  });

  describe('health', () => {
    it('should return healthy with DPC cluster info', async () => {
      constraintRuleRepo.query = jest.fn().mockResolvedValue([]);
  ...
  describe('Constitutional Compliance (Tier X)', () => {
    ...
    it('no business DB access (TXD-005)', () => {
      const svc = require('fs').readFileSync(require('path').join(__dirname, '..', 'application', 'services', 'design-constraint.service.ts'), 'utf8');
    ...
    it('event namespace uses designconst.* (ESR-003)', () => {
      const svc = require('fs').readFileSync(require('path').join(__dirname, '..', 'application', 'services', 'design-constraint.service.ts'), 'utf8');
      expect(svc).toContain("'designconst.");
    });
    it('API path is /api/v1/design-constraints (API-004)', () => {
