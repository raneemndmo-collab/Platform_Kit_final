// ═══════════════════════════════════════════════════════════════════════
// D7: Interaction Engine — Unit Tests | Phase 7 Tier X
// ═══════════════════════════════════════════════════════════════════════
...
describe('InteractionEngineService', () => {
  ...
  let interactionLayerRepo: ReturnType<typeof mockRepo>;
  let navigationMapRepo: ReturnType<typeof mockRepo>;
  let liveBindingRepo: ReturnType<typeof mockRepo>;
  let eventBus: typeof mockEventBus;

  beforeEach(async () => {
    interactionLayerRepo = mockRepo();
    navigationMapRepo = mockRepo();
    liveBindingRepo = mockRepo();
    eventBus = { publish: jest.fn(() => Promise.resolve()) };
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        InteractionEngineService,
        { provide: getRepositoryToken(InteractionLayer), useValue: interactionLayerRepo },
        { provide: getRepositoryToken(NavigationMap), useValue: navigationMapRepo },
        { provide: getRepositoryToken(LiveBinding), useValue: liveBindingRepo },
        { provide: 'EventBus', useValue: eventBus },
      ],
    }).compile();
    service = module.get<InteractionEngineService>(InteractionEngineService);
    (service as any).eventBus = eventBus;
  });

  describe('generateInteractions', () => {
    it('should create with tenant isolation (TXD-001, P-016)', async () => {
      const result = await service.generateInteractions('tenant-1', { name: 'Test' });
      expect(result).toBeDefined();
      expect(interactionLayerRepo.create).toHaveBeenCalledWith(expect.objectContaining({ tenantId: 'tenant-1' }));
    });
    it('should emit interact.* event (ESR-003)', async () => {
      await service.generateInteractions('tenant-1', { name: 'Test' });
      expect(eventBus.publish).toHaveBeenCalledWith(expect.stringContaining('interact.'), expect.any(Object));
    });
  });

  describe('health', () => {
    it('should return healthy with DPC cluster info', async () => {
      interactionLayerRepo.query = jest.fn().mockResolvedValue([]);
  ...
  describe('Constitutional Compliance (Tier X)', () => {
    ...
    it('no business DB access (TXD-005)', () => {
      const svc = require('fs').readFileSync(require('path').join(__dirname, '..', 'application', 'services', 'interaction-engine.service.ts'), 'utf8');
    ...
    it('event namespace uses interact.* (ESR-003)', () => {
      const svc = require('fs').readFileSync(require('path').join(__dirname, '..', 'application', 'services', 'interaction-engine.service.ts'), 'utf8');
      expect(svc).toContain("'interact.");
    });
    it('API path is /api/v1/interact (API-004)', () => {
